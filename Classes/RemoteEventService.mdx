...
## Functions
#### `PushToClient(string username, string eventName, object[] data)`
...
#### `PushToAllClients(string eventName, object[] data)`
<Info>This function can only be called by the server.</Info>
This function is used to send data from server to all connected clients. You can send unlimited parameters after desired first 2 parameters. An example:
```lua
local RemoteEventService = Game:GetService("RemoteEventService")
local data = {}
...
#### `SendEventToServer(string eventName, object[] data)`
...
function FunctionToCall(data, username, statement_by_client, numberSent, cubeByClient)
    ...
    printl("Cube type given by client: " .. cubeByClient.IsA()) -- outputs: Client type given by client: Instance
end
```
</CodeGroup>

#### `ConnectEvent(string eventName, object[] data)`
